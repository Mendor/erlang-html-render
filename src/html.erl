...
render({Name}) when is_atom(Name) ->
  start_tag(Name, []);
render({Name, AttrList, Content}) ->
  [start_tag(Name, AttrList), render(Content), end_tag(Name)];
render({Name, Content}) when is_atom(Name), is_binary(Content) ->
  [start_tag(Name, []), render(Content), end_tag(Name)];
render({Name, Content}) when is_atom(Name), is_tuple(Content) ->
  [start_tag(Name, []), render(Content), end_tag(Name)];
render({Name, AttrList}) when is_atom(Name) ->
  start_tag(Name, AttrList);
render(Input) when is_list(Input) ->
  [render(Term) || Term <- Input];
render(Input) when is_integer(Input) ->
  integer_to_list(Input);
render(Input) when is_atom(Input) ->
  atom_to_list(Input);
render(Input) when is_binary(Input) ->
  escape(Input).
